...
Для определения ОС узла можно пользоваться знанием TTL пакета при помощи команды **ping**, которая шлет ICMP пакеты Echo Request и получает в ответ Echo Reply, но это очень неточно. Более точный способ - утилита **nmap**. Но тоже, конечно, не со 100% уверенностью. Т.к. администратор по идее должен всячески скрывать версию ОС сервера для усложнения совершения атаки на него. 
...
1. 209.85.249.158 - 75% потерянных запросов
...
### 4. Сравнить результаты выполнения traceroute при использовании разных сетевых протоколов: IP, TCP, UDP.


#### To Google via UDP:
```
nuacho@nuacho-HP-EliteBook-840-G2:~$ traceroute google.com
...
#### To Google via ICMP:
```
nuacho@nuacho-HP-EliteBook-840-G2:~$ traceroute -I google.com
...
#### To Google via TCP:
```
nuacho@nuacho-HP-EliteBook-840-G2:~$ sudo traceroute -T -O info -p 80 google.com 
[sudo] password for nuacho: 
traceroute to google.com (173.194.220.101), 30 hops max, 60 byte packets
 1  _gateway (192.168.1.1)  3.345 ms  3.411 ms  3.399 ms
 2  * * *
 3  lag-8-435.bbr01.voronezh.ertelecom.ru (109.195.24.18)  3.787 ms  3.778 ms  3.769 ms
 4  72.14.215.165 (72.14.215.165)  17.923 ms  18.602 ms  17.903 ms
 5  72.14.215.166 (72.14.215.166)  18.582 ms  20.429 ms  17.868 ms
 6  108.170.250.130 (108.170.250.130)  18.546 ms 108.170.250.34 (108.170.250.34)  18.521 ms 108.170.250.113 (108.170.250.113)  18.288 ms
 7  * 209.85.255.136 (209.85.255.136)  32.619 ms *
 8  216.239.48.224 (216.239.48.224)  29.326 ms  29.297 ms 209.85.254.6 (209.85.254.6)  32.498 ms
 9  172.253.51.187 (172.253.51.187)  34.402 ms 209.85.254.135 (209.85.254.135)  31.615 ms 172.253.51.239 (172.253.51.239)  31.586 ms
10  * * *
11  * * *
12  * * *
13  * * *
...
17  * * *
18  * * *
19  * * lk-in-f101.1e100.net (173.194.220.101) <syn,ack>  29.905 ms

```
ICMP 

Второй узел настроен так, чтобы не отвечать на UDP дейтаграммы, но ICMP пакеты он обрабатывает. 
